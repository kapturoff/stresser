...
with concurrent.futures.ThreadPoolExecutor(max_workers=config["threads"]) as executor:
    ...
    print(
        f"Average speed was {round(total_requests_sent / duration)} requests per second."
    )
    print(
        f"Every thread was spending around {sum(averages_request_times) / len(averages_request_times)} ms for request."
    )
